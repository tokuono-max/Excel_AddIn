...
概要:
    ...
    本バージョンにて、未使用インポート（Any, hc_prg）を物理抹消し Ruff 警告を根絶。
    挿入先の行が持つ既存フォント設定を物理継承するロジックを実装。
    一切の要約を排し、1 命令 1 行の原子分解記述を徹底。

改訂履歴:
    1.0.1: 未使用インポート（Any, hc_prg）の抹消による Ruff 警告の解消。
    1.0.0: 新規分割作成。hc_header Ver 1.0.5 より挿入機能を分離・最適化。
"""
...
path_raw_src = os.path.abspath(__file__)
# 変数: 自分自身が属するディレクトリ。
path_dir_svc = os.path.dirname(path_raw_src)
# 変数: プロジェクトルート（svc の一つ上）のパスを算出。
path_root_prj = os.path.dirname(path_dir_svc)

# 判定コメント: 検索パスの先頭にプロジェクトルートが存在するか確認。
if path_root_prj not in sys.path:
    # 命令分離: 検索パスの先頭に物理挿入。
    # 【目的】パッケージとして実行する際や、VSCode での解析エラーを根絶するため。
    sys.path.insert(0, path_root_prj)

try:
    # 命令分離: Win32 制御、および Excel 操作の階層インポート。
...
def insert_header(target_hwnd: Optional[int] = None) -> None:
    ...
    # 変数: 親 HWND 解決。
    val_xl_h = 0
    # 判定コメント: 呼び出し元からハンドルが明示的に渡されているか。
    if target_hwnd is not None:
        # 命令分離: 整数値へのキャスト。
        val_xl_h = int(target_hwnd)
    else:
        # 【目的】引数不在時、OS 側から現在の Excel 窓を物理特定するため。
        # 命令分離: アプリポインタから動的に Win32 ID を原子抽出。
    ...
    try:
        ...
        # ---------------------------------------------------------
        # 【目的】既存行を押し下げ、1 行目に定型ヘッダを具現化する。

        # 命令分離: 操作権限のロック。
        # 【目的】書込中のユーザー介入による例外を物理防止するため。
        ...
        with xlc.Opt(ptr_a):
            ...
            # --- スタイルの原子リストア ---
            # 【目的】挿入に伴い自動付与された太字等の意図しない設定を平準化。
            try:
                # 変数: 新しく具現化した行の API オブジェクト。
        ...
        # --- 物理環境復旧 ---
        try:
            # 命令分離: インタラクティブ権限の原子復旧。
            ptr_a.api.Interactive = True
        except Exception:
            # 判定コメント: 断絶時は沈黙。
            pass

        # 命令分離: 本来の親 Excel へのフォーカス還流エンジンの執行。
        # 【重要】アドイン処理終了後、OS のフォーカス位置を安定させる。
