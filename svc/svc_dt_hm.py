...
def _dt_hm_trace(phase: str, t0: float, **kv: object) -> None:
    try:
        if kv:
            _dt_hm_diag.info(
                "[DT_HM_TRACE] phase=%s cumulative_ms=%d %s",
                phase,
                _elapsed_ms(t0),
                " ".join("%s=%s" % (k, v) for k, v in kv.items()),
            )
        else:
            _dt_hm_diag.info("[DT_HM_TRACE] phase=%s cumulative_ms=%d", phase, _elapsed_ms(t0))
    except Exception:
        pass


def _perf_dt_hm(phase: str, t0: float, **kv: object) -> None:
    try:
        if kv:
            _perf.info(
                "[DT_HM_PERF] phase=%s cumulative_ms=%d %s",
                phase,
                _elapsed_ms(t0),
                " ".join("%s=%s" % (k, v) for k, v in kv.items()),
            )
        else:
            _perf.info("[DT_HM_PERF] phase=%s cumulative_ms=%d", phase, _elapsed_ms(t0))
    except Exception:
        pass
...
def _cfg() -> dict[str, Any]:
    """
    日付・時刻変換用の画面・メッセージ設定を config/ui_dt_hm.json から読み込む。
...
def _progress_closed_ack_path(sheet_id: str) -> Path:
    d = Path(get_ipc_root()) / "progress"
    d.mkdir(parents=True, exist_ok=True)
    return d / f"progress_dt_hm_closed_{sheet_id}.pkl"
...
def _wait_progress_closed_ack(path: Optional[Path], timeout_sec: float = _PROGRESS_CLOSE_ACK_TIMEOUT_SEC) -> None:
    ...
    while True:
        ...
        if (time.perf_counter() - t0) >= max(0.05, float(timeout_sec)):
            logger.info("[DT_HM] progress close ack timeout: %s", str(p))
            return
...
def _submit_progress_ui(
    ...
    *,
    ...
    """
    UI サーバに進捗画面表示を依頼する。req_*.pkl に payload を書き、ui_server が ui_dt_hm.create_dialog を呼ぶ。
    """
    try:
        ...
        result_path = str(res_dir / f"res_progress_dt_hm_{ts_ms}_{os.getpid()}.pkl")
        ...
        payload = {
            ...
            "log_path": "",
            "action": "progress",
            "module": "ui_qt.ui_dt_hm",
            "req_dict": req_dict,
        }
        req_path = get_request_dir() / f"req_{ts_ms}_{os.getpid()}_{threading.get_ident()}.pkl"
        write_pickle(req_path, payload)
        logger.info("[DT_HM] progress UI request: %s", req_path)
    except Exception as exc:
        logger.warning("[DT_HM] progress UI request failed: %s", exc)


def _submit_done_ui(parent_hwnd: int, sheet_id: str, message: str, title: str = "日付・時刻変換") -> None:
    """
    ...
    try:
        ...
        res_dir = Path(get_ipc_root()) / "result"
        res_dir.mkdir(parents=True, exist_ok=True)
        result_path = str(res_dir / f"res_dt_hm_done_{ts_ms}_{os.getpid()}.pkl")
        req_dict: dict[str, Any] = {
            "action": "dt_hm_done",
            "modeless": False,
            "title": str(title),
        ...
        write_pickle(req_path, payload)
        logger.info("[DT_HM] done UI request: %s", req_path)
    except Exception as exc:
        logger.warning("[DT_HM] done UI request failed: %s", exc)


def _submit_warning_ui(parent_hwnd: int, sheet_id: str, message: str, title: str = "日付・時刻変換") -> None:
    """
    ...
    try:
        ...
        res_dir = Path(get_ipc_root()) / "result"
        res_dir.mkdir(parents=True, exist_ok=True)
        result_path = str(res_dir / f"res_dt_hm_warning_{ts_ms}_{os.getpid()}.pkl")
        req_dict: dict[str, Any] = {
            "action": "dt_hm_warning",
            "modeless": False,
            "title": str(title),
            "message": str(message),
        }
        ...
        write_pickle(req_path, payload)
        logger.info("[DT_HM] warning UI request: %s", req_path)
    except Exception as exc:
        logger.warning("[DT_HM] warning UI request failed: %s", exc)
...
def _sheet_id_resolve(ptr_s: Any, sheet_id: str) -> str:
    ...
    if core_xlc_mod is not None:
        try:
            ...
            pass
    return f"dt_hm_{abs(id(ptr_s))}"


def convert_date_ymd_hm(target_hwnd: Optional[int] = None, sheet_id: str = "") -> None:
    """
    【概要】
        指定 HWND の Excel ブック・シートにおいて、選択範囲の日付・時刻を YYYY/MM/DD HH:MM 形式へ一括変換する。
    【補足】
        進捗・完了通知は ui_server 経由で ui_dt_hm に依頼する。設定は config/ui_dt_hm.json。
    """
    t_flow = time.perf_counter()
    _perf_dt_hm("enter", t_flow)
    _dt_hm_trace("enter", t_flow)

    if core_xlc_mod is None:
        logger.error("[DT_HM] core_xlc not available")
        _perf_dt_hm("abort_no_core_xlc", t_flow)
        _dt_hm_trace("abort_no_core_xlc", t_flow)
        return
    ctx = core_xlc_mod.get_excel_context_from_hwnd(int(target_hwnd or 0), sheet_id)
    if ctx is None:
        logger.error("[DT_HM] Excel context not available (xlwings + HWND)")
        _perf_dt_hm("abort_no_context", t_flow)
        _dt_hm_trace("abort_no_context", t_flow)
        return

    ptr_a, ptr_w, ptr_s, ph = ctx
    logger.info("[DT_HM] 開始")
    _perf_dt_hm("after_context", t_flow, hwnd=ph)
    _dt_hm_trace("after_context", t_flow, hwnd=ph)
    cfg = _cfg()
    ...
    try:
        ...
        if ptr_sel is None:
            logger.info("[DT_HM] 選択範囲なし")
            _perf_dt_hm("early_no_selection", t_flow)
            _dt_hm_trace("early_no_selection", t_flow)
        ...
        _perf_dt_hm("after_progress_ui_submit", t_flow)
        _dt_hm_trace("after_progress_ui_submit", t_flow)
        ...
        _dt_hm_trace("after_matrix_read", t_flow, rows=processed_rows, cols=total_sel_cols)
        _perf_dt_hm("after_analyze", t_flow, success_n=val_success_n, non_empty=non_empty_count)
        _dt_hm_trace("after_analyze", t_flow, success_n=val_success_n, non_empty=non_empty_count)
