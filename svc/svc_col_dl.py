...
def _col_trace(phase: str, t0: float, **kv: object) -> None:
    try:
        if kv:
            _col_diag.info(
                "[COL_DL_TRACE] phase=%s cumulative_ms=%d %s",
                phase,
                _elapsed_ms(t0),
                " ".join("%s=%s" % (k, v) for k, v in kv.items()),
            )
        else:
            _col_diag.info("[COL_DL_TRACE] phase=%s cumulative_ms=%d", phase, _elapsed_ms(t0))
    except Exception:
        pass


def _perf_col(phase: str, t0: float, **kv: object) -> None:
    try:
        if kv:
            _perf.info(
                "[COL_DL_PERF] phase=%s cumulative_ms=%d %s",
                phase,
                _elapsed_ms(t0),
                " ".join("%s=%s" % (k, v) for k, v in kv.items()),
            )
        else:
            _perf.info("[COL_DL_PERF] phase=%s cumulative_ms=%d", phase, _elapsed_ms(t0))
    except Exception:
        pass
...
def _cfg() -> dict[str, Any]:
    """
    空白列削除用の画面・メッセージ設定を config/ui_col_dl.json から読み込む。
...
def _progress_path(sheet_id: str) -> Path:
    ...
    return d / f"progress_col_dl_{sheet_id}.pkl"
...
def _submit_progress_ui(
    ...
    *,
    ...
    """
    UI サーバに進捗画面表示を依頼する。req_*.pkl に payload を書き、ui_server が ui_col_dl.create_dialog を呼ぶ。
    """
    try:
        ...
        result_path = str(res_dir / f"res_progress_col_dl_{ts_ms}_{os.getpid()}.pkl")
        ...
        payload = {
            ...
            "log_path": "",
            "action": "progress",
            "module": "ui_qt.ui_col_dl",
            "req_dict": req_dict,
        }
        req_path = get_request_dir() / f"req_{ts_ms}_{os.getpid()}_{threading.get_ident()}.pkl"
        write_pickle(req_path, payload)
        logger.info("[COL_DL] progress UI request: %s", req_path)
    except Exception as exc:
        logger.warning("[COL_DL] progress UI request failed: %s", exc)


def _submit_done_ui(parent_hwnd: int, sheet_id: str, message: str, title: str = "空白列削除") -> Path | None:
    """
    完了通知をモーダルで表示するため ui_server に依頼する。
    戻り値は ui_server が書き込む result_path（待機用）。
    """
    try:
        from svc.svc_host import ensure_ui_server
        ...
        res_dir = Path(get_ipc_root()) / "result"
        res_dir.mkdir(parents=True, exist_ok=True)
        result_path = res_dir / f"res_col_dl_done_{ts_ms}_{os.getpid()}.pkl"
        req_dict: dict[str, Any] = {
            "action": "col_dl_done",
            "modeless": False,
            "title": str(title),
        ...
        write_pickle(req_path, payload)
        logger.info("[COL_DL] done UI request: %s", req_path)
        return result_path
    except Exception as exc:
        logger.warning("[COL_DL] done UI request failed: %s", exc)
        return None
...
def _submit_preview_ui(
    ...
    *,
    ...
    try:
        ...
        res_dir = Path(get_ipc_root()) / "result"
        res_dir.mkdir(parents=True, exist_ok=True)
        result_path = res_dir / f"res_col_dl_preview_{ts_ms}_{os.getpid()}.pkl"
        preview_cfg = (cfg.get("SCREENS") or {}).get("PREVIEW") or {}
        title = str(preview_cfg.get("TITLE") or "空白列削除")
        req_dict: dict[str, Any] = {
            "action": "col_dl_preview",
            "modeless": False,
        ...
        req_path = get_request_dir() / f"req_col_dl_preview_{ts_ms}_{os.getpid()}.pkl"
        write_pickle(req_path, payload)
        logger.info("[COL_DL] preview UI request: %s", req_path)
        return result_path
    except Exception as exc:
        logger.warning("[COL_DL] preview UI request failed: %s", exc)
        return None
...
def _read_sheet_matrix(
    ...
    *,
    ...
    cancel_path があればチャンク境界でキャンセルを検知し _ColDlCancelled を送出する。
    """
    msg_scan = _msg(cfg, "PHASE_SCAN")
    custom = (cfg.get("MESSAGES") or {}).get("PROGRESS_CUSTOM_SCAN") or "解析中"
...
def _sheet_id_resolve(ptr_s: Any, sheet_id: str) -> str:
    ...
    if core_xlc_mod is not None:
        try:
            ...
            pass
    return f"col_dl_{abs(id(ptr_s))}"


def delete_empty_cols(target_hwnd: Optional[int] = None, sheet_id: str = "") -> None:
    """
    【概要】
        指定 HWND の Excel ブック・アクティブシートの使用範囲内で、空白列を検出して物理削除する。
    【補足】
        進捗・完了通知は ui_server 経由で ui_col_dl に依頼する。設定は config/ui_col_dl.json。
    """
    t_flow = time.perf_counter()
    _perf_col("enter", t_flow)
    _col_trace("enter", t_flow)

    if core_xlc_mod is None:
        logger.error("[COL_DL] core_xlc not available")
        _perf_col("abort_no_core_xlc", t_flow)
        _col_trace("abort_no_core_xlc", t_flow)
        return
    ctx = core_xlc_mod.get_excel_context_from_hwnd(int(target_hwnd or 0), sheet_id)
    if ctx is None:
        logger.error("[COL_DL] Excel context not available (xlwings + HWND)")
        _perf_col("abort_no_context", t_flow)
        _col_trace("abort_no_context", t_flow)
        return

    ptr_a, ptr_w, ptr_s, ph = ctx
    logger.info("[COL_DL] 開始")
    _perf_col("after_context", t_flow, hwnd=ph)
    _col_trace("after_context", t_flow, hwnd=ph)
    cfg = _cfg()
    ...
    try:
        rect_u = ptr_s.used_range
        val_y_st = int(rect_u.row)
        val_x_st = int(rect_u.column)
        val_yn = int(rect_u.rows.count)
        val_xn = int(rect_u.columns.count)
        if val_xn < 1 or val_yn < 1:
            logger.info("[COL_DL] 使用範囲なし")
            _perf_col("early_no_used_range", t_flow)
            _col_trace("early_no_used_range", t_flow)
            return

        logger.info(
            "[COL_DL] 使用範囲 row=%s col=%s rows=%s cols=%s",
            val_y_st,
            val_x_st,
            val_yn,
            val_xn,
        )
        _perf_col("after_used_range", t_flow, rows=val_yn, cols=val_xn)
        _col_trace("after_used_range", t_flow, rows=val_yn, cols=val_xn)
        ...
        _perf_col("after_progress_ui_submit", t_flow)
        _col_trace("after_progress_ui_submit", t_flow)

        try:
            matrix = _read_sheet_matrix(
                ptr_s, val_y_st, val_x_st, val_yn, val_xn, _upd, cfg, cancel_path=cancel_path
            )
        except _ColDlCancelled:
            logger.info("[COL_DL] 走査キャンセル")
            _perf_col("user_cancel_scan", t_flow)
            _col_trace("user_cancel_scan", t_flow)
            _progress_write(prog_path, {"status": "DONE", "seq": 999})
            return

        if matrix is None:
            logger.warning("[COL_DL] 読込失敗")
            _perf_col("abort_matrix_read_failed", t_flow)
            _col_trace("abort_matrix_read_failed", t_flow)
            _progress_write(prog_path, {"status": "DONE", "seq": 999})
            ...
            rp_err = _submit_done_ui(
                ph, sid, _msg(cfg, "ERROR_PREFIX"), str(done_cfg.get("TITLE") or "空白列削除")
            )
            if rp_err is not None:
                _wait_ui_dispatch_result(rp_err)
            return

        _perf_col("after_matrix_read", t_flow, yn=val_yn, xn=val_xn)
        _col_trace("after_matrix_read", t_flow, yn=val_yn, xn=val_xn)

        list_col_apis: list[Any] = []
        list_col_indices: list[int] = []
        for j_ptr in range(val_xn):
            val_abs_x = val_x_st + j_ptr
            bool_is_c_empty = True
            for i_ptr in range(1, len(matrix)):
                val_c = matrix[i_ptr][j_ptr] if j_ptr < len(matrix[i_ptr]) else None
                if val_c is not None:
                    str_v = str(val_c)
                    if str_v.strip() != "":
                        bool_is_c_empty = False
                        break
            if bool_is_c_empty:
                ptr_range_obj = ptr_s.range((val_y_st, val_abs_x))
                list_col_apis.append(ptr_range_obj.api.EntireColumn)
                list_col_indices.append(val_abs_x)

        val_num_del = len(list_col_apis)
        if val_num_del >= 1:
            logger.info("[COL_DL] 空白列 件数=%s 位置(列)=%s", val_num_del, list_col_indices)

        _progress_write(prog_path, {"status": "DONE", "seq": 999})
        _ensure_progress_closed_before_modal()

        _done_screen = (cfg.get("SCREENS") or {}).get("DONE") or {}
        done_title = str(_done_screen.get("TITLE") or "空白列削除")

        if val_num_del == 0:
            logger.info("[COL_DL] 空白列なし")
            _perf_col("early_no_blank_cols", t_flow)
            _col_trace("early_no_blank_cols", t_flow)
        ...
        _perf_col("after_scan_blank_cols", t_flow, delete_count=val_num_del)
        _col_trace("after_scan_blank_cols", t_flow, delete_count=val_num_del)

        letters_preview = [_col_index_to_letters(c) for c in list_col_indices]
        rp_prev = _submit_preview_ui(
            ph, sid, kind="confirm", items=letters_preview, cfg=cfg
        )
        if rp_prev is None:
            return
        ...
        if choice != "delete":
            logger.info("[COL_DL] 削除を中止（確認: %s）", choice)
            _perf_col("user_cancel_confirm", t_flow, choice=choice)
            _col_trace("user_cancel_confirm", t_flow, choice=choice)
            return
        ...
        try:
            ...
            logger.warning("[COL_DL] save_undo_snapshot failed (undo unavailable): %s", e)

        from core import core_xlc

        with core_xlc.suspend_sheet_updates(ptr_s):
            for api_it in reversed(list_col_apis):
                api_it.Delete()

        positions_str = ", ".join(_col_index_to_letters(i) for i in list_col_indices)
        logger.info("[COL_DL] 完了 抹消=%s 位置(列)=%s", val_num_del, list_col_indices)
        _status_bar_set(ptr_w, _msg(cfg, "STATUS_DONE", count=val_num_del, positions=positions_str))
        rp_done = _submit_done_ui(
            ph,
            sid,
            _msg(cfg, "STATUS_DONE", count=val_num_del, positions=positions_str),
            done_title,
        )
        if rp_done is not None:
            _wait_ui_dispatch_result(rp_done)
        _perf_col("after_delete_done", t_flow, deleted=val_num_del)
        _col_trace("after_delete_done", t_flow, deleted=val_num_del)

    except Exception as ex:
        logger.exception("[COL_DL] %s", ex)
        try:
        ...
        try:
            ...
            pass
        _perf_col("flow_end", t_flow)
        _col_trace("flow_end", t_flow)
