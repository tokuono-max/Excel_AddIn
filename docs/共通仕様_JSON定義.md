...
実装は `ui_qt.ui_common._normalize_message_newlines`（ツールチップは `_normalize_tooltip_text` 経由）または `*_HTML` 用の `_dch`（`\n` → `<br/>`）を通るキーが対象。**キー名の接頭辞だけで自動適用されるわけではなく**、コードが当該キーを読み取り上記関数に渡す場合に有効。

| 接頭辞 / パターン | 主な用途 | 備考 |
|-------------------|----------|------|
| **TIP_** | ツールチップ | データ集約の `SCENARIO_EDIT` / `DEBUG` 等 |
| **TOOLTIP_** | ツールチップ | メイン画面・各機能共通 |
| **MSG_** | ダイアログ・通知の本文 | 例: `MSG_SCENARIO_LOAD_DONE_FMT` |
| **MSG**（完全一致） | `WARNING.MSG` 等 | オブジェクト内のキー名 |
| **MESSAGE** | 通知本文 | `MSG` と同趣旨 |
| **MSG_HEADER** | 完了・通知の見出し | DONE 画面等 |
| **MSG_*_PREFIX** | 件数・行数などの接頭辞 | `MSG_COUNT_PREFIX` 等 |
| **DESC** / **DESCRIPTION** | 画面説明文 | `DESC_VISIBLE` と併用 |
| **DETAIL_TEXT** | 詳細本文の固定部 | undo 等 |
| **LABEL_** | ラベル表示 | `QLabel` の折り返し設定次第 |
| **BTN_** | ボタン文言 | |
| **SEC_** | セクション見出し | |
| **PLACEHOLDER_** | 入力欄プレースホルダ | |
| **\*_HTML** | HTML 断片ラベル | `HINT_TOP_HTML` 等。`\n` は `<br/>` に変換 |
| **MESSAGES**（配下の値） | 機能別メッセージ一式 | トップレベル `MESSAGES` ブロック |
| **配列内の文字列** | コンボ・チェックの選択肢 | 例: `WRITE_MODE_ITEMS` |

**改行が効きにくい / 別扱い**

| キー | 備考 |
|------|------|
| **TITLE** | ウィンドウタイトル（1 行想定。改行は非推奨） |
| **VALUE_SHAPE_HINT_HTML** 等 | HTML 本文。**`<br/>` 推奨**（`*_HTML` 経由なら `\n` も可） |

上記以外でも、設定文字列を `_normalize_message_newlines` 経由で表示する箇所では同様に `\n` / `\\n` が有効（§6.3 の **DETAIL_TEXT**、ラジオ等の補助説明文など）。
