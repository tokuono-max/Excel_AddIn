# -*- coding: utf-8 -*-
"""Ver1.1.9.5 データ集約ドキュメント（Docx）生成。利用者（操作者）向け。"""
...
def build_ui_manual() -> Document:
    ...
    bullets(
        ...
        [
            ...
            "整形（DSL）：左から順にコマンドを当てる。書き方は別紙の整形 DSL 説明を見る。",
        ],
        numbered=True,
    )
    ...
    img_slot(doc, "図5-1", "デバッグの結果", "結果一覧。可能なら先頭に「・」が付いた列が見える状態。")
    ...
    h(doc, "6.3 見出し行を落とし、見出しの品名を明細へ回す", 2)
    table(
        doc,
        ["項目", "値"],
        [
    ...
    table(
        ...
        ["文書", "内容"],
        [
            ["主キー・連携キー・結合キーの動作概念_V1195", "3つのキーの違いと例"],
            ["シナリオファイルJson構成と概要_V1195", "保存ファイルの見方"],
            ["整形DSL_コマンドリファレンス_V1195", "文字の直し方"],
        ],
    )
    return doc
...
def build_dsl_doc() -> Document:
    ...
    img_slot(doc, "図1-1", "整形（DSL）の入力欄", "主キーブロックの「整形（DSL）」が見える状態。")
    ...
    img_slot(doc, "図6-1", "整形の設定と結果", "左：整形の入力。右：デバッグの結果。")
...
def main() -> None:
    save(build_ui_manual(), "データ集約_シナリオ編集画面説明書_V1195.docx")
    save(build_keys_concept(), "データ集約_主キー・連携キー・結合キーの動作概念_V1195.docx")
    save(build_json_doc(), "データ集約_シナリオファイルJson構成と概要_V1195.docx")
    save(build_dsl_doc(), "データ集約_整形DSL_コマンドリファレンス_V1195.docx")
