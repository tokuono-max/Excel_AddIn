...
> **更新（2026-08）**: 旧版 `full_prev` バックアップと半自動ロールバックは **廃止**（速度優先）。
> bin は **差分 → フル** のみ。失敗時は再試行／再インストール案内。
> 現行の正は `docs/インストールと運用（利用者・運用向け）.md` §3.4。
> 以下のバックアップ／復旧の記述は当時メモ。現行方針を優先すること。
...
- （旧）full zip バックアップ世代管理 → **現行は廃止**
- （旧）復旧方式（半自動） → **現行は廃止**（差分→フル／再インストール）
