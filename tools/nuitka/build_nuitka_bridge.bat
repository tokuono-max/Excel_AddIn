...
set "NUITKA_OUT=%CD%\%CSV_TOOL_STAGING%\app\_stage_bridge"
rem Nuitka outputs to a stage dir; merge_nuitka_stage_into_bin.bat copies into app\bin (single onedir).
...
set "LOGFILE=%LOGDIR%\build_bridge_%LOGTS%.log"
set "REMOVE_OUTPUT="
if /i "%NUITKA_REMOVE_OUTPUT%"=="1" set "REMOVE_OUTPUT=--remove-output"

echo [Nuitka] bridge (hc_main) -^> %NUITKA_OUT%  jobs=%NUITKA_JOBS%
echo [Nuitka] Log file: %LOGFILE%
"%PY%" "%~dp0nuitka_log_wrapper.py" "%LOGFILE%" hc_main.py ^
  --standalone ^
