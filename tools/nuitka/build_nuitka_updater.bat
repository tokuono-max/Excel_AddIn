...
set "NUITKA_OUT=%CD%\%CSV_TOOL_STAGING%\app\_stage_updater"
...
set "LOGFILE=%LOGDIR%\build_updater_%LOGTS%.log"
set "REMOVE_OUTPUT="
if /i "%NUITKA_REMOVE_OUTPUT%"=="1" set "REMOVE_OUTPUT=--remove-output"

echo [Nuitka] updater (hc_updater) -^> %NUITKA_OUT%  jobs=%NUITKA_JOBS%
echo [Nuitka] Log file: %LOGFILE%
"%PY%" "%~dp0nuitka_log_wrapper.py" "%LOGFILE%" hc_updater.py ^
  --standalone ^
  ...
  %REMOVE_OUTPUT% ^
  --output-filename=hc_updater.exe ^
  --show-progress ^
  --jobs=%NUITKA_JOBS%
