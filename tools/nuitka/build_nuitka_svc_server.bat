...
set "NUITKA_OUT=%CD%\%CSV_TOOL_STAGING%\app\_stage_svc_server"
...
set "LOGFILE=%LOGDIR%\build_svc_server_%LOGTS%.log"
set "REMOVE_OUTPUT="
if /i "%NUITKA_REMOVE_OUTPUT%"=="1" set "REMOVE_OUTPUT=--remove-output"

echo [Nuitka] svc_server -^> %NUITKA_OUT%  jobs=%NUITKA_JOBS%
echo [Nuitka] Log file: %LOGFILE%
"%PY%" "%~dp0nuitka_log_wrapper.py" "%LOGFILE%" svc\svc_server.py ^
  --standalone ^
