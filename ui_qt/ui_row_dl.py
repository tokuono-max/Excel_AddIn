...
"""
Python: 3.12
Module: ui_qt/ui_row_dl.py
Created: 2026-03-06
Updated: 2026-05-05
Version: 1.1.9
Purpose:
  空白行削除の UI（進捗・完了通知）。設定は config/ui_row_dl.json 必須。
History (latest 3):
...
def _get_cfg() -> dict[str, Any]:
    """
    空白行削除用の画面設定を config/ui_row_dl.json から読み込む。
...
class _RowDlProgressWrapper:
    """
    進捗ダイアログ（ui_common.create_progress_dialog の戻り値）をラップし、
    show / get_result を svc_row_dl 側から扱いやすくする。
    """
...
class _RowDlPreviewDialog(QDialog):
    """
    空欄行の一覧と削除／キャンセル、または空欄なし時の OK のみ。
    showEvent で Excel HWND を無効化するため、closeEvent で有効化（svc finally との二重は可）。
    """
    ...
    def __init__(
        ...
        title = str(self._req.get("title") or self._preview_cfg.get("TITLE") or "空白行削除").strip()
        ...
        # hdr.setMinimumWidth(320)
        hdr.setMinimumWidth(250)
...
class _RowDlDoneDialog(QDialog):
    """
    空白行削除完了時の通知をモーダルで表示するダイアログ。
    ...
    def __init__(
        ...
        title = str(self._req.get("title") or self._done_cfg.get("TITLE") or "空白行削除").strip()
        self.setWindowTitle(title)
    ...
    def showEvent(self, event) -> None:
        ...
        try:
            ...
            log_ui_fg_phase(
                "row_dl_done_showEvent_after_super",
                self._parent_hwnd,
                self,
...
def create_dialog(
    ...
    parent_hwnd: int,
    sheet_id: str,
) -> _RowDlProgressWrapper | _RowDlDoneDialog | _RowDlPreviewDialog:
    """
    【概要】
        ui_server から呼ばれ、action に応じて進捗または完了通知ダイアログを生成する。
    【補足】
        設定は config/ui_row_dl.json。progress / row_dl_done の各 action を処理する。
    """
    req = req_dict or {}
    ...
    if action == "progress":
        ...
        return _RowDlProgressWrapper(dlg)

    if action == "row_dl_done":
        cfg = _get_cfg()
        done_cfg = (cfg.get("SCREENS") or {}).get("DONE") or {}
        dlg = _RowDlDoneDialog(req, int(parent_hwnd or 0), str(sheet_id or ""), done_cfg)
        try:
    ...
    if action == "row_dl_preview":
        cfg = _get_cfg()
        preview_cfg = (cfg.get("SCREENS") or {}).get("PREVIEW") or {}
        dlg = _RowDlPreviewDialog(req, int(parent_hwnd or 0), str(sheet_id or ""), preview_cfg)
        ph = int(parent_hwnd or 0)
        try:
            from ui_qt.ui_common import excel_rect_tuple_from_req, prepare_dialog_excel_center_before_show
    ...
    raise ValueError(f"ui_row_dl: unknown action {action!r}")
