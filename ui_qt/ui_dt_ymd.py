...
"""
Python: 3.12
Module: ui_qt/ui_dt_ymd.py
Created: 2026-03-06
Updated: 2026-05-02
Version: 1.1.1
Purpose:
  日付変換（YYYY/MM/DD）の UI（進捗・完了通知）。設定は config/ui_dt_ymd.json 必須。
History (latest 3):
  ...
  - 1.0.0 (2026-03-18) hc_dt_ymd から分離。進捗は ui_common、完了は SCREENS.DONE で表示。
  - 初出 (2026-03-06) 計画に基づく ui_dt_ymd 新規作成。
"""
...
def _get_cfg() -> dict[str, Any]:
    """
    日付変換用の画面設定を config/ui_dt_ymd.json から読み込む。
...
class _DtYmdProgressWrapper:
    """
    進捗ダイアログ（ui_common.create_progress_dialog の戻り値）をラップし、
    show / get_result を svc_dt_ymd 側から扱いやすくする。
    """
...
class _DtYmdDoneDialog(QDialog):
    """
    日付変換完了時の通知をモーダルで表示するダイアログ。
    ...
    def __init__(
        ...
        title = str(self._req.get("title") or self._done_cfg.get("TITLE") or "日付変換").strip()
...
def create_dialog(
    ...
    parent_hwnd: int,
    sheet_id: str,
) -> _DtYmdProgressWrapper | _DtYmdDoneDialog:
    """
    【概要】
        ui_server から呼ばれ、action に応じて進捗または完了通知ダイアログを生成する。
    【補足】
        設定は config/ui_dt_ymd.json。progress / dt_ymd_done の各 action を処理する。
    """
    req = req_dict or {}
    ...
    if action == "progress":
        ...
        return _DtYmdProgressWrapper(dlg)

    if action == "dt_ymd_done":
        cfg = _get_cfg()
        from ui_qt.ui_common import (
    ...
    raise ValueError(f"ui_dt_ymd: unknown action {action!r}")
